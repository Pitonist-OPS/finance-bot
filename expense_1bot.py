...
# === НАСТРОЙКИ ===
import os
BOT_TOKEN = os.getenv("BOT_TOKEN")  # ← ЗАМЕНИТЕ НА СВОЙ!

bot = Bot(token=BOT_TOKEN)
dp = Dispatcher()

# Хранилище выбранных дат (упрощённо)
user_selections = {}
    ...
    async with aiosqlite.connect("expenses.db") as db:
        await db.execute("""
            CREATE TABLE IF NOT EXISTS expenses (
                ...
                amount REAL NOT NULL,  -- ← Теперь REAL, а не INTEGER!
...
async def generate_report_for_period(chat_id: int, start_date: date, end_date: date):
    ...
    for cat, amt in summary.items():
        # Округляем до 2 знаков при выводе
...
async def cmd_start(message: types.Message):
    await message.answer(
        "Привет! Отправляй расходы в формате:\n<code>34.5р еда</code> или <code>120,99р кино</code>\n\n"
        "Используй кнопки ниже для анализа.",
...
async def handle_expense(message: types.Message):
    ...
    match = re.match(r'^([\d,\.]+)\s*[рруб]*\s+(.+)$', text)

    if not match:
        await message.answer(
            "Не понял. Примеры:\n<code>34.5р еда</code>\n<code>120,99р кино</code>",
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        await message.answer("❌ Неверный формат суммы. Используйте цифры, например: 34.5 или 120,99")
        return
    ...
    display_amount = f"{amount:.2f}".rstrip('0').rstrip('.')
    await message.answer(f"✅ Добавлено: {display_amount}р — {category}")
